...
renamed_file="$workdir/renamed.safetensors"
...
"$moon_bin" run src/cmd/sftk -- query "$source_file" 'prefix=a,dtype=U8,rank=1' --format json | grep -F '"name":"a"' >/dev/null
...
"$moon_bin" run src/cmd/sftk -- rename-prefix "$source_file" --from-prefix a --to-prefix encoder.a --output "$renamed_file" --write --format json | grep -F '"changed":true' >/dev/null
"$moon_bin" run src/cmd/sftk -- validate "$renamed_file" --format json | grep -F '"valid":true' >/dev/null
"$moon_bin" run src/cmd/sftk -- query "$renamed_file" 'prefix=encoder.a,dtype=U8,rank=1' --format json | grep -F '"name":"encoder.a"' >/dev/null
